...
% Test met production with FBA or TFA
%
% USAGE:
%
%    [BBB, BBBprod, essBBB, slack4BBB, model, rxnsUsed] = checkBBBTasks(model, definecase, essThr, MetsToDrain, printSol, testslack, grRate)
% 
% INPUT:
%    model:           COBRA model structure including rxn-reaction associations
%
% OPTIONAL INPUTS:
%    definecase:      Either 'FBA' or 'TFA' (Default = 'FBA')
%    essThr:          Threshold on met production (% of grRate)
%                     below which we dont consider production (default = 0.1)
%    MetsToDrain:     Mets to test production (default = BBBs)
%    printSol:        Print solutions used to allow production of met
%                     (default = false - only for FBA)
%    testslack:       Get slacks to allow production of met in TFA (default
%                     = false - only for TFA)
%    grRate:          Growth rate expected (default = 0.07)
%
%
%
% OUTPUTS:
%    BBB:             Summary of mets whose production was tested
%    BBBprod:         Max production of met achieved (mmol/gDW/h)
%    essBBB:          Mets whose production is below the threshold defined
%    slack4BBB:       Slacks for met whose production with TFA was not feasible
%    model:           Model used for analysis (this will be useful when
%                     other formulations are integrated to test met production)
%    rxnsUsed:        List of reactions used to allow met production (one
%                     of all alternative solutions)
%
% .. Author:
